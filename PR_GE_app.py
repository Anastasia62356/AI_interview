...
def AI_QU():
    ...
    prompt = f"""
        ...
        【目的】
        以下の自己PR文をもとに、候補者の真実性（再現性）、深さ（思考力）、応用力（ポテンシャル）を見抜くための質問を作成します。

        【絶対ルール】
        1. 質問は **{question_count}個ちょうど** 出力すること。
        2. 各質問の前に必ず **「1.」「2.」...** のように半角数字＋ピリオドで番号を付けること。
        3. 質問文以外（説明文、挨拶、タイトル、補足、区切り線、空行など）は一切出力してはいけません。
        4. 各質問は1文以内、句点（。)で終わること。
        5. 出力フォーマットは以下の例と**完全に一致**させること。

        【出力例（質問数が3の場合）】
        1. この自己PRで述べた行動を取るきっかけは何でしたか。
        2. 数値的成果をどのように測定しましたか。
        3. 同様の状況で再現するために意識していることは何ですか。
        ...
        【出力タスク】
        上記を基に、**{question_count}個の質問**を正確に出力してください。
        """
...
def AI_EV():
    ...
    **以下の「評価項目」に基づき、100点満点で採点・フィードバック・改善案の生成を行ってください。
