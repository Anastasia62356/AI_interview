...
def PR_GE():
    ...
    【絶対厳守ルール】
    1. **生成する文章は、**上限**{max_char_count}字**を**絶対に超えてはいけません**。さらに、**{min_char_count}字以上**に収めることを**絶対厳守**してください。
...
def AI_QU():
   ...
   prompt = f"""あなたは、ユーザーの成功のために、**文字数と全ての制約を絶対厳守するプロフェッショナルなキャリアコンサルタント**です。
